...
export default function Decryption(
    ...
    let encryptionWord = encryption.split(":"),
        encryptionWordNumber = encryptionWord.map((v) => parseInt(v)),
        styleASCII = wordToASCII(style),
        tmp: number[] = [],
        el = encryptionWordNumber.length,
        l = styleASCII.length - 1,
        en = encryptionWord.length < styleASCII.length;
    ...
    if (!en)
        ...
        let repeatTimes =
            parseInt(`${styleASCII.length / encryptionWordNumber.length}`) - 1;
